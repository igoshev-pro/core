...
type ApiKind = "layouts";
type ExtendedProps = LayoutUpsertSectionMainProps & { api?: ApiKind };

export default function LayoutUpsertSectionMainC({
	type,
	projectId,
	id,
	api = "layouts",
	...
	const ops = useMemo(() => {
		switch (api) {
			case "layouts":
				return { get: getLayout, create: createLayout, update: updateLayout };
			// case "superAdmin":
			// case "user":
			default:
				return { get: getLayout, create: createLayout, update: updateLayout };
		...
		ops
			...
			.catch(() => {
				addToast({
					color: "danger",
					title: "Ошибка!",
					description: `Не удалось загрузить макет`,
					variant: "solid",
					radius: "lg",
					timeout: 3000,
					shouldShowTimeoutProgress: true,
		...
		if (type === UpsertType.Create) {
			...
			try {
				...
				addToast({
					color: "success",
					title: "Успешно!",
					description: `Макет создан успешно`,
					variant: "solid",
					radius: "lg",
					timeout: 3000,
					shouldShowTimeoutProgress: true,
				});
				router.push(`/admin/factory/layouts`);
			} catch {
				addToast({
					color: "danger",
					title: "Ошибка!",
					description: `Произошла ошибка при создании макета`,
					variant: "solid",
					radius: "lg",
					timeout: 3000,
					shouldShowTimeoutProgress: true,
				...
				<>
					<div className="flex items-center gap-6 mb-9">
						...
						<h1 className="text-3xl font-bold">
							{type === UpsertType.Create ? "Создание макета" : "Редактирование макета"}{" "}
