...
const formSchema = z.object({
	...
	emails: z
		.array(z.object({ value: z.string().email("Некорректный email") }))
		.optional(),
...
export default function ProjectSiteSettingsEditPage({
	...
	const { url: logoDarkRemoteUrl } = usePresignedUrl(
		settings?.companyLogoDarkPath
	);
	const { url: logoAltRemoteUrl } = usePresignedUrl(
		settings?.companyLogoAltPath
	);
	...
	const [logoDarkPreviewUrl, setLogoDarkPreviewUrl] = useState<string | null>(
		null
	);
	const [logoAltPreviewUrl, setLogoAltPreviewUrl] = useState<string | null>(
		null
	);
	...
	const uploadToSettings = async (
		...
		const nextSettings = {
			...
			...(kind === "companyLogoDark"
				? { companyLogoDarkPath: uploaded.path }
				: {}),
			...(kind === "companyLogoAlt"
				? { companyLogoAltPath: uploaded.path }
				: {}),
				...
				reset(
					{
						...
						emails: (contacts?.emails ?? [""]).map((e: string) => ({
							value: e,
						})),
						phones: (contacts?.phones ?? ["+7"]).map((p: string) => ({
							value: p,
						})),
					},
	...
	const onSubmit = async (data: FormData) => {
		...
		const nextSettings = {
			...
			companyLogoPath:
				(item?.settings?.companyLogoPath ?? data.companyLogoPath) || undefined,
			companyLogoDarkPath:
				(item?.settings?.companyLogoDarkPath ?? data.companyLogoDarkPath) ||
				undefined,
			companyLogoAltPath:
				(item?.settings?.companyLogoAltPath ?? data.companyLogoAltPath) ||
				undefined,
			faviconPath:
				(item?.settings?.faviconPath ?? data.faviconPath) || undefined,

			seoDefaults: {
				title:
					buildI18n3(data.seo_title_ru, data.seo_title_en, data.seo_title_de) ??
					{},
									...
									{phonesFA.fields.map((f, idx) => (
										...
										>
											<Input
												...
												className="flex-1"
												isInvalid={Boolean(
													(errors.phones as any)?.[idx]?.value
												)}
												errorMessage={
													(errors.phones as any)?.[idx]?.value?.message
												}
									...
									{emailsFA.fields.map((f, idx) => (
										...
										>
											<Input
												...
												className="flex-1"
												isInvalid={Boolean(
													(errors.emails as any)?.[idx]?.value
												)}
												errorMessage={
													(errors.emails as any)?.[idx]?.value?.message
												}
