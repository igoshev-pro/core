...
export class ProjectController {
  ...
  findByDomain(@Param('domain') domain: string) {
    return this.projectService.findByDomain(domain);
  }

  @Get('for-render/:domain')
  async getForRender(@Param('domain') domain: string) {
    const res = await this.projectService.getForRender(domain)
    console.log(res)
    return res
  }
